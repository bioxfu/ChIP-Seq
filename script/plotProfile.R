...
ymin <- 0.5
ymax <- max(as.numeric(chip[3,])) + 0.5
plot(as.numeric(chip[2,]), as.numeric(chip[3,]), type='n', ylim=c(ymin, ymax), xlab='', ylab='Normalized read coverage', xaxt='n', yaxt='n', cex.lab=1.2)
abline(h=seq(ymin, ymax, 0.5), col='gray', lwd=0.5)
abline(v=seq(0, 300, 50), col='gray', lwd=0.5)
lines(as.numeric(chip[2,]), as.numeric(chip[3,]), lwd=3, col=col_set[1])
#lines(input[2,], input[3,], lwd=3, col=col_set[2])
axis(1, lab, c('-1000', 'start', 'end', '1000'), cex.axis=1.2)
axis(2, seq(ymin, ymax+0.5, 0.5), seq(ymin, ymax+0.5, 0.5), las=2, cex.axis=1.2)
